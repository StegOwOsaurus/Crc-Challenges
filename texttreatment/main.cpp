...
inline int to_int(string num){
    ...
    for(int i = 0; i < num.length(); i++){
        thing += (num[i]-48)*pow(10.0, num.length()-(i+1));
    }
    return thing;
}

string testOp(string word){
    char operators[] = {'+', '-', '*', '/'};
    bool convert = false;
    for(int i = 0; i < word.length(); i++){
        for(int j = 0; j < 4; j++){
            if(word[i] == operators[j]){
                ...
                int num1 = to_int(word.substr(0,i)), num2 = to_int(word.substr(i+1, word.length()-1));
...
string solve(string testCase){
    ...
    for(int i = 0; i < testCase.length(); i++){
        std::string word;
        while(testCase[i] != ' ' && i < testCase.length()){
            word+=testCase[i++];
        }
        word =  testOp(word);
	...
	return answer;   //Return test case output here / Retourner la sortie de votre code sur cette ligne
}
...
int main(int argc, char *argv[]){
	...
	//inputs / entrées
	string TEST_CASES [nbr_cases] = {"dans l'ordre naturel des choses, 22+20 a toujours ete un nombre important. peu importe le contexte, 42 represente la verite vraie.", "etant riche de 3*5 dollars, julie peut s'acheter les bonbons de son choix! son ami jimmy, lui, possede 14-12 dollars et n'aura donc pas ses bonbons desires. il devra se contenter d'acheter de la gomme.", "charles goes to his school every morning in montreal, quebec, knowing that his dreams are about to be fulfilled.","what a beautiful day outside! it's almost like louis could touch some grass! 20/5 kilos on his lawn, to be precise."};
	/* Looks like this in problem's description: / A l'air de ça dans la description du problème:
	...
	*/
	...
	string EXPECTED_ANSWERS [nbr_cases] = {"Dans l'or_dre naturel des cho_ses, 42 a touj_ours ete\tu_n\tnom_bre important. Peu importe l_e cont_exte, 42 repre_sente\tl_a ver_ite vraie.\t", "Etant riche d_e 15 dollars, Julie pe_ut s'ach_eter les bonbons d_e son choix! Son ami Jimmy, lui, possede 2 dollars e_t\tn'aura\tdo_nc pas ses bonbons desires. Il devra\ts_e contenter d'ach_eter d_e l_a gomme.", "Charles go_es t_o\this sch_ool every\tmorning\ti_n Montreal, Quebec, knowing th_at his dre_ams\tare about t_o\tb_e fulfilled.", "What a beautiful day\toutside! It's alm_ost\tli_ke Louis could\ttouch so_me grass! 4 kilos o_n his la_wn,\tt_o	b_e precise.\t"};
	/* Should look like this after the checkAnswer() method: / Devrait ressembler à ça après la fonction checkAnswer():
